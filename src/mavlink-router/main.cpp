...
static int parse_argv(int argc, char *argv[])
{
    ...
    while ((c = getopt_long(argc, argv, short_options, long_options, NULL)) >= 0) {
        ...
        case 'e': {
            ...
            add_endpoint_address(NULL, 0, ip, port, false, NULL, true);
            free(ip);
            break;
        }
        case 'r': {
    ...
    while (optind < argc) {
        ...
        if (stat(base, &st) == -1 || !S_ISCHR(st.st_mode)) {
            ...
            add_endpoint_address(NULL, 0, base, number, true, NULL, true);
        } else {
        ...
        if (ret == 0) {
            if (opt_udp.eavesdropping && opt_udp.port == ULONG_MAX) {
                ...
                ret = add_endpoint_address(iter.name + offset, iter.name_len - offset, opt_udp.addr,
                                           opt_udp.port, opt_udp.eavesdropping, opt_udp.filter, opt_udp.portlock);
            }
        }
